...
class RTFMessage{
  ...
  String createPacket(){
    ...
    packet += RTF_Substates.sent_value.hexVal;
    packet += padding(attributeCount,bit2);

    packet += attribute(RTFAttributes.RTF_data.hexVal,bit8s,padding(value, bit8));
  ...
  String attributeName(String t){
    ...
    if(t == RTFAttributes.RTF_data.hexVal){
      return RTFAttributes.RTF_data.name;
    }
    else{
      return "";
    }
  ...
  String receiveRequest(){
    ...
    packet = Separator.sof.hexVal+"08"+Information.RTF.hexVal+RTF_Substates.request.hexVal+"00"+Separator.eof.hexVal;
